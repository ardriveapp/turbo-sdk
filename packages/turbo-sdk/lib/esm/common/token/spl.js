...
export class SplToken {
    constructor({ mintAddress, decimals, tokenName = 'SPL', gatewayUrl = defaultProdGatewayUrls.solana, logger = Logger.default, pollingOptions = {
        maxAttempts: 10,
    ...
    async createAndSubmitTx({ target, signer, tokenAmount, turboCreditDestinationAddress, }) {
        ...
        tx.add(createTransferCheckedInstruction(fromAta, mint, toAta, ownerPublicKey, BigInt(new BigNumber(tokenAmount).toFixed(0)), this.decimals));
