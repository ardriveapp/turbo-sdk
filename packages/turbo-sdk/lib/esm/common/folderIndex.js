...
import { contentHashFromFolderIndexKey, contentHashTagName, folderIndexKey, isValidContentHash, isValidFolderIndexKey, } from '../utils/folderIndex.js';
/**
...
function resolveOwnerAddress(owner) {
    const fromPublicKey = (publicKey) => {
        if (typeof publicKey === 'string' && !/^[a-zA-Z0-9_-]+$/.test(publicKey)) {
            throw new ProvidedInputError('createChainFolderIndex owner.publicKey must be base64url, or the raw bytes from await turbo.signer.getPublicKey()');
        }
        ...
        if (!publicKeyByteLengths.has(raw.length)) {
            throw new ProvidedInputError(`createChainFolderIndex owner.publicKey must be an ed25519 (32 byte), secp256k1 (65 byte) or RSA (512 byte) public key, got ${raw.length} bytes. Pass await turbo.signer.getPublicKey(), or use { address } if what you have is an owner address.`);
        }
    ...
    if (owner !== null && typeof owner === 'object') {
        ...
        if (typeof owner.address === 'string') {
            if (!isValidArweaveBase64URL(owner.address)) {
                throw new ProvidedInputError(`createChainFolderIndex owner.address must be a 43 character base64url address, got '${owner.address}'. A native address (an 0x... or a base58 Solana address) is not what a gateway indexes uploads under.`);
            }
            return owner.address;
        }
    }
    throw new ProvidedInputError('createChainFolderIndex needs owner: await turbo.signer.getPublicKey(), or { publicKey } or { address }. ' +
...
function requirePositiveInteger(value, name, max) {
    ...
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > max) {
        throw new ProvidedInputError(`createChainFolderIndex ${name} must be an integer between 1 and ${max}, got '${value}'`);
    }
    return parsed;
}
/**
...
const contentHashesOf = (keys) => new Set([...keys].map(contentHashFromFolderIndexKey));
/**
 ...
 */
export function createMemoryFolderIndex(seed = {}) {
    const map = new Map(Object.entries(seed).filter(([key, id]) => isValidFolderIndexKey(key) && isValidArweaveBase64URL(id)));
    return {
        name: 'memory',
        get: (key) => map.get(key),
        set: (key, id) => {
 ...
 */
export function createChainFolderIndex({ owner, appName, gatewayUrl = 'https://arweave.net', hashTagName = contentHashTagName, maxPages = 20, pageSize = 100, timeoutMs = 30_000, fetchImpl = fetch, logger, }) {
 ...
 */
export function composeFolderIndex(layers, { logger } = {}) {
    ...
    if (declaredHashTagNames.length > 1) {
        throw new ProvidedInputError(`composeFolderIndex layers disagree about which tag holds a file's content hash: ${declaredHashTagNames
            .map((name) => `'${name}'`)
